...
public class FileService {
	 ...
	 * @return 저장된 파일명
	 */
	public static String saveFile(MultipartFile mfile, String uploadPath, HttpServletRequest req) {
		...
		String resourcesPath = req.getSession().getServletContext().getRealPath("/resources/images/");
		...
		try {
			run.exec("cmd.exe chcp 65001"); // cmd에서 한글문제로 썸네일이 만들어지지않을시 cmd창에서
											// utf-8로 변환하는 명령
			run.exec(command);
		} catch (Exception e) {
		...
		while (true) {
			File f = new File(thumnail + ".png");
			if(f.exists()){
				System.out.println("이미지 저장 주소: "+f.getAbsolutePath());
				break;
			}
		}
